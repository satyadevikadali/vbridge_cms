...
class CustomtablesModelTables extends JModelAdmin
{
	...
	function getRenameTableIfNeeded($tableid,$database,$dbprefix,$tablename)
	{
		...
		{
				//rename table
			$tablestatus=$this->getTableStatus($database,$dbprefix,$old_tablename);
	...
	function getTableStatus($database,$dbprefix,$tablename)
	{
		$db = JFactory::getDBO();
	...
	function processDBTable($database,$dbprefix,$tablename,$tabletitle)
	{
		...
		$rows2=$this->getTableStatus($database,$dbprefix,$tablename);
