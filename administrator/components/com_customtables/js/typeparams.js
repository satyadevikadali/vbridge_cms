...
function updateParameters()
{
    ...
    if(typeparams!=null)
    {
        ...
        updateParamString('fieldtype_param_',1,param_array.length,typeparams_id,null);
    }
    else
...
function renderParamBox(typeparams,typeparams_box,paramvaluestring)
{
    ...
    var values=parseQuote(paramvaluestring,',',true);
            ...
            for(var i=0;i<param_array.length;i++)
            {
                ...
                if (proversion || typeof(param_att.proversion) === "undefined" || param_att.proversion==="0")
                {
                    ...
                    var attributes='onchange="updateParamString(\'fieldtype_param_\',1,'+param_array.length+',\''+typeparams_box+'\',event);"';

                    result+=renderInputBox('fieldtype_param_'+i,param,vlu,attributes);

                    //if (typeof(param.description) != "undefined")
                       // result+='<p style="font-style:italic;">'+param.description+'</p>';
    ...
    //jQuery(function($){
        //ct_initPopovers(null,null);




        //$("body").on("subform-row-add", initPopovers);
        //function initPopovers (event, container) { $(container || document).find(".hasPopover").popover({"html": true,"trigger": "hover focus","container": "body"});} });

    //current_params_count=;

    //if(typeparams_box!="")
    //{
        //var obj=document.getElementById(typeparams_box);

        //if(obj!=null && typeof obj === 'object')
        //{
            //obj.innerHTML=result;
            //updateParamString(typeparams_box,null);
        //}
    //}
    //else
...
function updateParamString(inputboxid,countlist,countparams,objectid,e)
{
...
function renderInput_Radio(objname,param,value,onchange)
    ...
    function renderInput_ImageSizeList(id,param,value,attributes)
    {
        ...
        //{
        //    temp_imagesizelist.push(value_array[i].split(','));
        //}
            ...
            if(count_list>0)
            {
                ...
                for(var r=0;r<count_list;r++)
                {
                    ...
                    for(i=0;i<count_params;i++)
                    {
                        ...
                        var attributes='onchange="updateParamString_ImageSizes(\'size_param_\','+count_list+','+count_params+',\''+temp_imagesize_parambox_id+'\',event);"';
    ...
    function updateParamString_ImageSizes(sizeparam,countlist,countparams,tempimagesizeparamboxid,e)
    {
        ...
        updateParamString(sizeparam,countlist,countparams,tempimagesizeparamboxid,e);
        ...
        //alert(countlist_);
        //alert(countparams_);
        //alert(tempimagesizeparamboxid_);

        updateParamString(sizeparam_,countlist_,countparams_,tempimagesizeparamboxid_,null);
